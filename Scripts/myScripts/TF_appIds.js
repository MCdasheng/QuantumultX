...
$.appIds = $.getdata("tf_appIds") ? $.getdata("tf_appIds") : "";

const reg = /^https:\/\/testflight\.apple\.com\/v3\/accounts\/(.*)\/ru\/(.*)/;
...
if (reg.test($request.url)) {
  ...
  $.setdata(account_key, "tf_account_key");
  ...
  $.msg($.name, `🎉添加${appId}成功!`, `当前appIds: ${$.getdata("tf_appIds")}`);
  $.done();
}
