...
async function getUnionPayUrlIntl() {
    const dateStr = new Intl.DateTimeFormat("en-CA", {
        timeZone: "Asia/Singapore",
    }).format(new Date());
    const yyyymmdd = dateStr.replace(/-/g, "");
    return `https://www.unionpayintl.com/upload/jfimg/${yyyymmdd}.json`;
}

async function checkExchangeRate() {
    const url = await getUnionPayUrlIntl();
    $.log(url);
    ...
    return $.http.get(options).then((resp) => {
        ...
        const rates = data.exchangeRateJson || [];

        const unionPayRate = findRate(rates, "SGD", "CNY");
        const unionPayIntlRate = (1 / parseFloat(findRate(rates, "CNY", "SGD"))).toFixed(9);

        if (unionPayRate && unionPayIntlRate) {
            const unionPayText = `银联汇率 SGD->CNY: ${unionPayRate}`;
            const unionPayIntlText = `银联国际 SGD->CNY: ${unionPayIntlRate}`;

            $.log(unionPayText);
            $.log(unionPayIntlText);
            $.msg("银联汇率", unionPayText, unionPayIntlText);
        } else {
            $.log(resp.body);
            $.msg("银联汇率", "获取汇率失败");
        }
...
function findRate(rates, transCur, baseCur) {
    const item = rates.find(
        (rate) => rate.transCur === transCur && rate.baseCur === baseCur
    );
    return item ? item.rateData : "";
}
