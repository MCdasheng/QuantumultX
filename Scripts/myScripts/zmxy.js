...
var signIn_url = $.getdata("zmxy_signIn_url");
var calendar_url = $.getdata("zmxy_calendar_url");
var session = $.getdata("zmxy_session");
var MSG1 = "";
var MSG2 = "";
...
async function calendar() {
    let options = JSON.parse(session);
    options.url = calendar_url;

    const now = new Date();
    const utc8Time = new Date(now.getTime() + 8 * 60 * 60 * 1000);
    const month = utc8Time.getUTCMonth() + 1;
    const day = utc8Time.getUTCDate();
    const formatted = `${month}月${day}号`;

    return $.http.get(options).then((resp) => {
        var obj = JSON.parse(resp.body);
        var msg = `今天是${formatted}\,已签到${obj.signinDays}天`;
        if (msg) {
            $.log(msg);
            $.log(resp.body);
            MSG2 = msg;
