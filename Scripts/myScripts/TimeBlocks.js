...
$.key = $.getdata("TimeBlocks_key");
$.times = $.getdata("TimeBlocks_times") ? $.getdata("TimeBlocks_times") : 10;

if (!$.key) {
  $.log("⚠️邀请信息为空!");
  $.log("请通过重写获取信息");
  $.msg($.name, "⚠️邀请信息为空!", "请通过重写获取信息");
  $.done();
}
...
async function a() {
  ...
  let options = {
    url: `https://timeblocks.com/api/users/join`,
    headers: {
    ...
    body: `ver=5.3.3&passwd=12345678&aType=E&name=adad&lang=zh&deviceId=1935b15b662b1913&email=${rd}%40qqq.com&key=${$.key}`,
  };
