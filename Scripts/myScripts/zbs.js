/* 
微信小程序: 植白说官方商城
手动签到一次, 抓 https://www.kozbs.com/demo/wx/home/signDay?userId=xxx
url中 userId数值 填入 "zbs_userId"
请求头中 X-Dts-Token 填入 "zbs_token"
url = https://raw.githubusercontent.com/MCdasheng/QuantumultX/main/Scripts/myScripts/zbs.js
*/

const $ = new Env("植白说");

const token = $.getdata("zbs_token");
const userId = $.getdata("zbs_userId");
...
const headers = {
  ...
  "X-Dts-Token": `${token}`,
  Host: `www.kozbs.com`,
  "User-Agent": `Mozilla/5.0 (iPhone; CPU iPhone OS 16_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.42(0x18002a2a) NetType/WIFI Language/zh_CN`,
};
...
async function sign() {
  let options = {
    url: `https://www.kozbs.com/demo/wx/home/signDay?userId=${userId}`,
    headers: headers,
  };
  return $.http.get(options).then((resp) => {
    ...
    if (obj.errno == 0) {
      desc = `🎉签到成功: ${obj.data.signCount}分`;
      asset = `💰积分余额: ${obj.data.integral}分`;
    } else {
      desc = "🔴失败";
      $.log(resp.body);
    }
