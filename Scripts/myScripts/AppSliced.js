/*
脚本功能: 获取AppSliced每日限免
29 10,16 * * * https://raw.githubusercontent.com/MCdasheng/QuantumultX/main/Scripts/myScripts/AppSliced.js, tag=AppSliced今日限免, img-url=https://raw.githubusercontent.com/Semporia/Hand-Painted-icon/master/Universal/Final.png, enabled=true
*/

const $ = new Env("AppSliced");
...
function a() {
  ...
  return $.http.get(options).then(
    (resp) => {
      $.log("🚧正在获取appsliced今日限免...\n");
      // $.log(resp.body);

      // link
      var links = [];
      var reg_link =
        /https\:\/\/itunes\.apple\.com\/app\/id(.*)\?at=11l3KP&mt=8&uo=4&ct=1/;
      var a = resp.body.match(
        /https\:\/\/itunes\.apple\.com\/app\/id(.*)\?at=11l3KP&mt=8&uo=4&ct=1/g
      );
      if (a.length != -1) {
        //   $.log("🎉获取app信息成功!");
        for (var i = 0; i < a.length; i++) {
          var b = reg_link.exec(a[i]);
          var link = `https://apps.apple.com/us/app/id${b[1]}\n`;
          // $.log(link);
          links.push(link);
        }
      } else {
      ...
      // name
      var names = [];
      var reg_name = /\<h2 class=\"app_list\"\>(.*)\<\/h2\>/;
      var c = resp.body.match(/\<h2 class=\"app_list\"\>(.*)\<\/h2\>/g);
      //   $.log(c.length);
      if (c.length != -1) {
        //   $.log("🎉获取app信息成功!");
        for (var j = 0; j < c.length; j++) {
          var d = reg_name.exec(c[j]);
          // $.log(d);
          // $.log(typeof d);
          var name = `🟢${d[1]}`;
          //   $.log(name);
