/*
脚本功能: 每日订阅
33 10 * * * https://raw.githubusercontent.com/MCdasheng/QuantumultX/main/Scripts/myScripts/proxy_daily.js, tag=每日订阅, img-url=https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/Color/Final.png, enabled=true
*/

const $ = new Env("每日订阅");

const tagName = ["fastFly", "jcbb", "caihong", "KFC"];
const domain = [
...
(async function () {
  await getSubscribe(tagName[0], domain[0]);
  await getSubscribe(tagName[1], domain[1]);
  await getSubscribe(tagName[2], domain[2]);
  await getSubscribe(tagName[3], domain[3]);
})()
  .catch((e) => $.log(e))
  .finally(() => {
    $.log("-----------------------------");
    var sub =
      ...
      encodeURIComponent(`{"server_remote":${JSON.stringify(subTotal)}}`);
    $.log(sub);
    $.msg("每日订阅", "🎉获取订阅链接成功!", sub);
    $.done();
  });

function register(tagName, domain) {
...
async function getSubscribe(tagName, domain) {
  $.log(`🟢正在获取${tagName}订阅...`);
  const subscribe_url = domain + `/api/v1/user/getSubscribe`;
  const auth = await register(tagName, domain);
