/*
脚本功能: 漫漫游签到
29 10 * * * https://raw.githubusercontent.com/MCdasheng/QuantumultX/main/Scripts/myScripts/mmy.js, img-url=figure.disc.sports.system, enabled=true
*/

const $ = new Env("mmy");

const cookie = $.getdata("mmy_cookie");

if (!cookie) {
  $.msg("cookie为空,请检查cookie!");
  $.done();
}
...
function getFormhash() {
  ...
  return $.http.get(options).then(
    (resp) => {
      ...
      // $.log(resp.body);
      var formhash = "";
      var reg1 = /formhash=(.*)\"\>退出/;
      var reg2 = /name=\"formhash\" value=\"(.*)\"/;

      if (resp.body.search(reg1) != -1) {
        formhash = reg1.exec(resp.body)[1];
      }
      if (resp.body.search(reg2) != -1) {
        formhash = reg2.exec(resp.body)[1];
      }
...
async function signIn() {
  ...
  let options = {
    url: `https://www.mmybt.com/plugin.php?id=dsu_paulsign:sign&operation=qiandao&infloat=1&sign_as=1&inajax=1`,
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36 Edg/110.0.1587.63",
      cookie: cookie,
    },
    body: `formhash=${formhash}&qdxq=kx&qdmode=3&todaysay=&fastreply=0`,
  };
  ...
  return $.http.post(options).then(
    (resp) => {
      ...
      // $.log(resp.body);
      var notice = "";
      var exp1 = "您今日已经签到，请明天再来！";
      var exp2 = "恭喜你签到成功!";

      if (resp.body.search(exp1) != -1) {
        notice = exp1;
      }
      if (resp.body.search(exp2) != -1) {
        notice = exp2;
      }
