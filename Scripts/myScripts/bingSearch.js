...
地区选择:
  ...
  外区额外有每日任务,连续完成一定天数可以获得额外积分
    目前只能手做任务,有时候还会失败        (未解决,手做任务都失败写锤子)
兑换物品:
...
async function mbSearch() {
  ...
  if (!$.mb_cookie) {
    ...
    let options = {
      url: `https://${$.host}/search?q=${rd}&PC=OPALIOS&form=LWS001&ssp=1&cc=JP&setlang=zh-hans&darkschemeovr=0&safesearch=moderate`,
      headers: {
...
async function mbSearch() {
  $.log("mbSearching...");
  const mb_cookie = $.getdata("bingSearchCookieMobileKey");
  ...
  if (!mb_cookie) {
    ...
    let rd = Math.random().toString(36).slice(-8);
    let options = {
      url: `https://www.bing.com/search?q=${rd}&PC=OPALIOS&form=LWS001&ssp=1&cc=JP&setlang=zh-hans&darkschemeovr=0&safesearch=moderate`,
      headers: {
        Accept: `text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8`,
        "Sapphire-DeviceType": `iPhone12`,
        Connection: `keep-alive`,
...
async function pcSearch() {
  $.log("pcSearching...");
  const pc_cookie = $.getdata("bingSearchCookiePCKey");
