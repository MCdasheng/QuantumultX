/*
url = https://raw.githubusercontent.com/MCdasheng/QuantumultX/main/Scripts/glados.js
From https://gist.githubusercontent.com/ddgksf2013/32b3d37d78433a34370cbfb69780160d/raw/glados.js
Glados官网注册地址：https://glados.rocks/landing/IISIV-2J7A9-V4M6L-JOTA8
签到一天获得一天，利用脚本，理论可无限白嫖
...
async function signin() {
  return new Promise((e) => {
    ...
    $.post(t, (t, s, a) => {
      try {
        console.log(a), ($.signBody = a), (signStatus = s.statusCode);
      } catch (n) {
        $.logErr(n, s);
      } finally {
        e();
...
async function showmsg() {
  if ("200" == signStatus) {
    let e = JSON.parse($.signBody);
    "Please Try Tomorrow" == e.message
      ? ($.subt = "🟢重复签到，")
      : ($.subt = "🎉签到成功，"),
      e.list.length > 0 &&
        ($.subt += "📅剩余天数：" + Math.round(e.list[0].balance));
  } else ($.subt = "🔴签到失败"), ($.msr = "");
  $.msg($.name, $.subt, "");
}
isGetCookie
  ? (async () => {
      ...
      (e.url = $request.url),
        ...
        $.setdata(JSON.stringify(e), $.signKey)
          ? ($.subt = `🎉获取Glados会话: 成功!`)
          : ($.subt = `🔴获取Glados会话: 失败!`),
        $.msg($.name, $.subt, "");
    })()
